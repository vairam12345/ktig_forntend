...
const handlePrevSlide = (totalSlides) => {
  ...
  useEffect(() => {
    ...
    axios.get(" http://15.206.228.110:8081/api/images/images").then((response) => {
      setForm1Events(response.data);
    });
    axios.get("http://15.206.228.110:8081/api/blogimages/images").then((response) => {
  ...
  const handleDelete = (formType, id) => {
    const apiUrl =
      formType === "form1"  
        ? `http://15.206.228.110:8081/api/images/images/${id}`
